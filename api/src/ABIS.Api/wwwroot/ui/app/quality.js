...
async function loadQaDefects() {
    ...
    try {
        ...
        if (!r.ok) {
            setErr(`Skid defects failed (${r.status}).`);
            return;
        }
        const rows = await r.json();
        $('#qaSub').textContent = `${rows.length} record${rows.length === 1 ? '' : 's'} on order ${job}`;
        const code = (n, desc) => n == null ? '' : desc ? `${esc(n)} — ${esc(desc)}` : `${esc(n)}`;
        ...
        setErr(`Skid defects failed: ${e.message}`);
    }
}
...
(async () => {
    ...
    $('#qaForm').addEventListener('submit', (e) => { e.preventDefault(); void loadQaDefects(); });
