...
const nWt = (v) => (v == null ? '0' : Number(v).toLocaleString(undefined, { maximumFractionDigits: 0 }));
function scaffold() {
    return `
  <div class="page">
...
function tileColors(status) {
    if (status === 'in-spec')
        return 'border-color:var(--ok,#3a3);background:var(--ok-soft,rgba(40,160,70,.1))';
    if (status === 'out-of-spec')
        return 'border-color:var(--crit,#d33);background:var(--crit-soft,rgba(200,0,0,.1))';
...
function renderBoard(b) {
    ...
    if (!skids.length) {
        $('#board').innerHTML = `<div class="muted">No skids on job ${esc(b.abJobNum)}.</div>`;
        return;
    }
...
(async () => {
    ...
    $('#fJob').addEventListener('keydown', (e) => { if (e.key === 'Enter')
        void load(); });
})();
