...
const equipPath = (p) => [p.systemEquipment, p.subsystemEquipment, p.itemDevice].filter(Boolean).join(' › ') || '—';
const dOnly = (d) => (d == null ? '' : d.toLocaleDateString());
const dInput = (d) => (d == null ? '' : new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 10));
...
function completeAndChecklist(p) {
    return `
    ...
    <p class="muted" style="margin:6px 0 0">Leave <em>next due</em> blank to advance automatically from ${p.daysBetween ? `days-between (${esc(p.daysBetween)})` : p.numOfTimesPerYear ? `${esc(p.numOfTimesPerYear)}×/year` : 'the interval — this PM has none, so the stored date is kept'}.</p>
...
async function fillEquipmentPickers(p) {
...
async function deletePm() {
    if (pmEditingId == null)
        return;
    if (!confirm(`Delete PM #${pmEditingId} and its checklist? (Refused if completions exist — retire it instead.)`))
        return;
    setErr('');
    setBusy(true);
    try {
...
async function completePm() {
    if (pmEditingId == null)
        return;
    if (!v('#cBy')) {
        setErr('Completed by is required.');
        return;
    }
    setErr('');
    setBusy(true);
...
async function loadActions(pmId) {
    ...
    document.querySelectorAll('#tActions [data-del-action]').forEach((b) => b.addEventListener('click', () => void removeAction(pmId, Number(b.dataset.delAction))));
}
async function addAction() {
    if (pmEditingId == null || !v('#aItem')) {
...
async function removeAction(pmId, actionId) {
    try {
        await client().deletePmAction(pmId, actionId);
...
(async () => {
    ...
    const allPms = async () => {
        ...
        for (let page = 1; page <= 50; page++) { // 50 x 200 = 10,000, far above any real PM count
            ...
            const items = r.items ?? [];
            out.push(...items);
            if (out.length >= (r.totalCount ?? 0) || items.length === 0)
                break;
        }
        return out;
    };
    const exportPms = async (kind) => {
        setBusy(true);
        try {
