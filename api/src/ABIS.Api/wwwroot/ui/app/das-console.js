...
async function loadJob() {
    ...
    try {
        ...
        // actually waiting on.
        void renderSketch({ card: $('#sketchCard'), meta: $('#sketchMeta'), img: $('#sketchImg'), link: $('#sketchLink') }, j.sketchId, undefined, j.sketchJobNote);
...
async function coilRunAction(path, body, okMsg, then) {
    if (lineNum == null) {
...
async function endCoil(endWeight, supervisorOverrideId) {
    await coilRunAction(`/api/das/lines/${lineNum}/coil-run/end`, { endWeight, endStatus: Number(v('#opEndStatus')), coilAbcNum: runCoil ?? undefined, abJobNum: job,
        supervisorOverrideId: supervisorOverrideId ?? undefined }, (r) => `Coil run ended${supervisorOverrideId != null ? ' (supervisor override)' : ''}${r.jobFinished ? ' — job finished' : ''}`, (r) => void showRecap(r.run));
}
async function showRecap(run) {
    const box = $('#opRecap');
    if (!run) {
        box.hidden = true;
        return;
...
async function doScan(raw) {
    if (job == null) {
        setErr('Load a job first — a scan resolves against that job’s coils.');
        return;
    }
    if (!raw.trim()) {
        scanned = null;
...
async function confirmScannedCoil() {
    const s = scanned;
    if (!s || s.outcome !== 'Resolved' || s.coilAbcNum == null)
        return;
    const wt = v('#scanWt');
    setBusy(true);
    try {
        if (wt !== '') {
            ...
            if (!r.ok) {
                setErr('Actual weight must be greater than 100 and less than 99999 lb.');
                return;
            }
        }
        ...
        await coilRunAction(`/api/das/lines/${lineNum}/coil-run/start`, { coilAbcNum: s.coilAbcNum, abJobNum: job }, () => `Coil ${s.coilAbcNum} scanned in — run open`);
        scanned = null;
        renderScan();
        setV('#scanBox', '');
    }
...
(async () => {
    ...
    $('#btnEndCoilOverride').addEventListener('click', () => {
        const wt = v('#opEndWt');
        if (wt === '') {
            setErr('Enter the weight left on the coil first — the supervisor is authorising THAT figure.');
            return;
        }
        void (async () => {
