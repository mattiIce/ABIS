 ...
 */
function wireSearch(top) {
    const input = top.querySelector('#shSearch');
    const panel = top.querySelector('#shSearchRes');
    if (!input || !panel)
        return;
    let sel = -1;
    const links = () => Array.from(panel.querySelectorAll('a'));
    const paint = () => links().forEach((a, i) => a.classList.toggle('sel', i === sel));
    const close = () => {
        panel.hidden = true;
        panel.innerHTML = '';
        sel = -1;
    ...
    document.addEventListener('mousedown', (e) => {
        const t = e.target;
        if (panel.hidden || !(t instanceof Node))
            return;
        if (!panel.contains(t) && !input.contains(t))
            close();
    });
}
/**
 ...
 */
export function applyDeepLink(selector) {
