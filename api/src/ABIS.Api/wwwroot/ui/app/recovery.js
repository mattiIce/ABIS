...
const exportCsv = (tb) => download(`${tb.name}.csv`, toCsv(tb), 'text/csv');
...
const exportExcel = (tb) => exportXlsx(tb.name, tb.name.slice(0, 31), tb.headers, tb.rows);
...
async function loadWorksheet(job, coil) {
    const cust = Number($("#cust").value);
    if (!cust) {
        $("#wks").innerHTML = "<p class=\"muted\">Pick a recovery customer first.</p>";
        return;
    }
    wksJob = job;
    wksCoil = coil;
    try {
        const w = await (await authFetch(`/api/recovery/jobs/${job}/coils/${coil}/worksheet?customerId=${cust}`)).json();
        lastWks = w;
      ...
      <div class="frow" style="margin-top:10px;align-items:center">
        ...
        $("#wks-csv")?.addEventListener("click", () => { if (lastWks)
            exportCsv(worksheetTable(lastWks)); });
        $("#wks-xlsx")?.addEventListener("click", () => { if (lastWks)
...
async function saveWorksheet() {
    ...
    const lines = Array.from(document.querySelectorAll("#wks tbody tr[data-t]")).map((tr) => ({
        ...
        netWt: Number(tr.querySelector(".w-wt").value) || 0,
        pieces: Number(tr.querySelector(".w-pc").value) || 0,
    }));
    try {
        ...
        if (!r.ok) {
            $("#wks").insertAdjacentHTML("beforeend", `<p class="err">Save failed (${r.status}).</p>`);
            return;
        }
...
async function loadRecoveryCustomers() {
    try {
        const rows = await (await authFetch("/api/quality/recovery-customers")).json();
        const seen = new Set();
...
async function load() {
    ...
    try {
        ...
        lastDefects = defects;
        lastJob = job;
        $('#rep-csv').disabled = report.length === 0;
        $('#rep-xlsx').disabled = report.length === 0;
        $('#par-xlsx').disabled = defects.length === 0;
        ...
        document.querySelectorAll('#rep-rows tr[data-coil]').forEach((tr) => tr.addEventListener('click', () => void loadWorksheet(job, Number(tr.getAttribute('data-coil')))));
