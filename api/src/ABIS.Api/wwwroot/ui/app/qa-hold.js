...
async function transition(kind) {
    ...
    if (!coil) {
        setErr('Coil ABC # is required.');
        return;
    }
    if (!by || !note) {
        setErr('Checked by and a reason/note are both required.');
        return;
    }
    setErr('');
    setOk('');
    ...
    try {
        ...
        if (!r.ok) {
            ...
            if (r.status === 404)
                why = `coil ${coil} not found`;
            else if (r.status === 409) {
                const p = await r.json().catch(() => null);
                why = p?.detail ?? p?.title ?? 'not allowed in the coil’s current state';
            }
            else if (r.status === 400)
                why = 'checked-by and note are required';
...
(async () => {
    ...
    $('#rows').addEventListener('click', (e) => {
        const t = e.target.closest('[data-coil]');
        if (!t)
            return;
        const coil = Number(t.getAttribute('data-coil'));
        if (coil) {
            $('#fCoil').value = String(coil);
            void loadHistory(coil);
        }
    });
    await loadHeld();
})();
