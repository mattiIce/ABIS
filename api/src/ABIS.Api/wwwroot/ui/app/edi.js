...
function renderValve(p) {
    ...
    $('#xValve').innerHTML = p.valveOpen
       ...
       also be armed.</p>`;
    if (p.valveOpen)
        $('#btnClose').addEventListener('click', () => void setValve(false));
    else
        $('#btnOpen').addEventListener('click', () => void setValve(true));
}
function renderArmed(p) {
    const rows = p.armed ?? [];
    ...
    const nameOf = (id) => partners.find((x) => x.customerId === id)?.customerName ?? '';
    ...
    document.querySelectorAll('#tArm .aDis').forEach((b) => b.addEventListener('click', () => void setArm(b.dataset.t ?? '', Number(b.dataset.c), false, '')));
}
async function loadTransmit() {
    try {
...
async function setValve(open) {
    const note = open ? val('#vNote') : '';
    if (open && !note) {
        setErr('Say why the valve is being opened - it is recorded against your name.');
        return;
    }
    if (open && !confirm('Open the EDI transmit valve?\n\nArmed partner/document pairs will be permitted '
        + 'to transmit. Anything legacy also sends will reach the partner twice.'))
        return;
    setBusy(true);
    setErr('');
    try {
        await client().setEdiTransmitValve(new EdiValveWrite({ open, note }));
        await loadTransmit();
    }
    catch (e) {
...
async function setArm(transactionType, customerId, armed, note) {
    setBusy(true);
    setErr('');
    $('#armMsg').innerHTML = '';
    try {
        ...
        if (r?.warning)
            $('#armMsg').innerHTML = alertBox('crit', ICON_LIVE, 'Legacy still sends this', esc(r.warning));
        await loadTransmit();
    }
    catch (e) {
...
(async () => {
    ...
    $('#armForm').addEventListener('submit', (e) => {
        ...
        if (!(cust > 0)) {
            setErr('A customer is required - arming is always for a named partner.');
            return;
        }
        void setArm($('#aType').value, cust, true, val('#aNote'));
    });
    showTab('tx');
