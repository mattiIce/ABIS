...
async function loadPath() {
    ...
    const hasConveyor = (l) => (live.get(l.lineNum)?.configured ?? false) || (l.skids ?? []).some(onPath);
    const shown = lines.filter(hasConveyor);
    const omitted = lines.length - shown.length;
    if (!shown.length) {
        box.innerHTML = `<span class="muted">No line here has a stacker conveyor${edgeUp ? '' : ' (and the edge line feed is unreachable, so live cells can\'t be read)'}.</span>`;
    ...
    $('#pathSub').textContent = (!edgeUp ? 'edge line feed unreachable — showing recorded positions only (which the stacker automation writes)'
        : !anyCells ? 'no conveyor cells mapped on the edge — showing recorded positions only (see Edge:Opc:ConveyorCellsByLine)'
            : anyLive || anyStack ? 'where each finished stack is between the stacker and the end of the wrapper line — live from the PLC cell sensors'
                : 'belt clear — no stack on any conveyor station right now') + note;
}
