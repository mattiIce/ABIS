...
async function loadJobs() {
    ...
    try {
      ...
      <tr class="click" data-id="${j.scheduledJobId}">
        ...
        $('#rows').querySelectorAll('button[data-toggle]').forEach((b) => b.addEventListener('click', (e) => { e.stopPropagation(); void toggle(Number(b.dataset.toggle), b.dataset.on === '1'); }));
        $('#rows').querySelectorAll('button[data-runs]').forEach((b) => b.addEventListener('click', (e) => { e.stopPropagation(); void loadRuns(Number(b.dataset.runs)); }));
...
async function toggle(id, enable) {
    ...
    try {
        if (enable)
            await client().enableScheduledJob(id);
        else
            await client().disableScheduledJob(id);
        await loadJobs();
    }
    catch (e) {
...
async function create() {
    ...
    const body = new ScheduledJobWrite({
        jobName: $('#nName').value.trim() || undefined,
        cronExpression: $('#nCron').value.trim() || undefined,
        targetOperation: $('#nOp').value.trim() || undefined,
        source: $('#nSource').value.trim() || undefined,
