...
const privChip = (p) => `<span class="chip ${Number(p) >= 1 ? 'ok' : 'mut'}">${Number(p) >= 1 ? 'Write' : 'ReadOnly'}</span>`;
let curUser = null;
let curGroup = null;
function scaffold() {
    return `
...
async function openUser(userId) {
    ...
    try {
        ...
        setV('#edLogin', u.loginId);
        setV('#edFirst', u.userFirstName);
        setV('#edLast', u.userLastName);
        setV('#edStatus', u.userStatus);
        $('#pwOk').textContent = '';
        $('#edOk').textContent = '';
        $('#pinOk').textContent = '';
        void loadPinState();
...
async function removeUserGrant(applicationId) {
    if (curUser == null)
        return;
    setErr('');
    setBusy(true);
...
async function loadPinState() {
    ...
    try {
        ...
        if (!r.ok) {
            box.innerHTML = '<span class="muted">PIN status unavailable.</span>';
            return;
        }
        const s = await r.json();
        if (!s.hasPin) {
            box.innerHTML = '<span><b>PIN</b>none — cannot authorise overrides</span>';
            return;
        }
...
async function clearPin() {
    ...
    if (!window.confirm('Remove this PIN? They can no longer authorise overrides. What they already authorised stays in the log.'))
        return;
    setErr('');
    setBusy(true);
    try {
...
async function createUser() {
    ...
    if (!login) {
        setErr('A login is required.');
        return;
    }
    if (!v('#nuFirst') && !v('#nuLast')) {
    ...
    try {
        ...
        if (!ok)
            return;
        ['#nuLogin', '#nuFirst', '#nuLast'].forEach((i) => setV(i, ''));
        setV('#nuStatus', '1');
...
async function removeUser() {
    ...
    if (!window.confirm(`Remove user ${v('#edLogin') || curUser}? This deletes their grants, groups and password.`))
        return;
    setErr('');
    setBusy(true);
    try {
...
async function loadGroups() {
    try {
      ...
      <td><button class="btn sm ghost delGrp" data-id="${g.userGroupId}" data-name="${esc(g.groupName)}" type="button">delete</button></td></tr>`).join('')
        ...
        document.querySelectorAll('#tAllGroups .grp').forEach((el) => el.addEventListener('click', () => void openGroup(Number(el.dataset.id))));
...
async function openGroup(groupId) {
    ...
    try {
        const [grants, members] = await Promise.all([
            getJson(`/api/security/groups/${groupId}/applications`),
            getJson(`/api/security/groups/${groupId}/members`),
        ]);
      ...
      <td><button class="btn sm ghost rmGrpGrant" data-a="${p.applicationId}" type="button">remove</button></td></tr>`).join('')
        ...
        document.querySelectorAll('#tGrpGrants .rmGrpGrant').forEach((b) => b.addEventListener('click', () => void removeGroupGrant(Number(b.dataset.a))));
...
async function createGroup() {
    const name = v('#ngName');
    if (!name) {
        setErr('A group name is required.');
        return;
    }
    setErr('');
    setBusy(true);
...
async function deleteGroup(groupId, name) {
    if (!window.confirm(`Delete group ${name || groupId}? Members are unlinked and its feature grants are removed.`))
        return;
    setErr('');
    setBusy(true);
    try {
        if (!await send(`/api/security/groups/${groupId}`, 'DELETE'))
            return;
        if (curGroup === groupId) {
            curGroup = null;
...
async function setGroupGrant() {
    if (curGroup == null) {
        setErr('Open a group first.');
        return;
    }
    const aid = v('#grpGrantAppId');
    if (!aid)
        return;
    setErr('');
    setBusy(true);
    try {
        if (await send(`/api/security/groups/${curGroup}/applications/${aid}`, 'PUT', { privilege: Number(v('#grpGrantPriv')) || 0 })) {
            await openGroup(curGroup);
            $('#grpOk').textContent = '✓ Grant set.';
        }
    }
    finally {
        setBusy(false);
...
async function removeGroupGrant(applicationId) {
    if (curGroup == null)
        return;
    setErr('');
    setBusy(true);
    try {
        if (await send(`/api/security/groups/${curGroup}/applications/${applicationId}`, 'DELETE')) {
            await openGroup(curGroup);
            $('#grpOk').textContent = '✓ Grant cleared.';
        }
    }
    finally {
        setBusy(false);
...
async function createFeature() {
    const name = v('#nfName');
    if (!name) {
        setErr('A feature name is required.');
        return;
    }
    setErr('');
    setBusy(true);
...
async function deleteFeature(applicationId, name) {
    if (!window.confirm(`Delete feature ${name || applicationId}? It is removed from every user and group grant.`))
        return;
    setErr('');
    setBusy(true);
    try {
        if (!await send(`/api/security/applications/${applicationId}`, 'DELETE'))
            return; // 409 for User Control surfaces via #err
        await Promise.all([loadApps(), loadFeatureOptions()]);
    }
    finally {
        setBusy(false);
