...
async function searchCoils() {
    ...
    try {
        ...
        if (v('#fCoilCust'))
            qs.set('customerId', v('#fCoilCust'));
        if (v('#fCoilSearch'))
            qs.set('search', v('#fCoilSearch'));
        if ($('#fReadyOnly').checked)
            qs.set('readyOnly', 'true');
        const r = await authFetch(`/api/coil-ownership/transferable-coils?${qs.toString()}`);
        if (!r.ok) {
            setErr(`Coil search failed (${r.status}).`);
            return;
        }
      ...
      <tr class="click" data-coil="${c.coilAbcNum}" data-owner="${c.customerId ?? ''}">
        ...
        document.querySelectorAll('#tCoils tr.click').forEach((tr) => tr.addEventListener('click', (e) => {
            if (e.target.closest('.ck'))
                return; // a checkbox click shouldn't pick the row
...
async function markReady() {
    const ids = Array.from(document.querySelectorAll('#tCoils .ck:checked')).map((c) => Number(c.dataset.coil));
    if (!ids.length) {
        setErr('Check one or more coils to mark ready.');
        return;
    }
    setErr('');
    $('#okReady').textContent = '';
    ...
    try {
        ...
        if (!r.ok) {
            setErr(`Mark ready failed (${r.status}).`);
            return;
        }
