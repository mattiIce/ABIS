 ...
 */
export async function renderSketch(target, sketchId, sketchName, jobNote) {
    const prev = shown.get(target.img);
    if (prev) {
        URL.revokeObjectURL(prev);
        shown.delete(target.img);
    }
    if (sketchId == null) {
    ...
    try {
        ...
        if (r.status === 404) {
            target.meta.innerHTML = meta('<span class="muted">No drawing stored for this sketch.</span>');
            return;
        }
        if (!r.ok) {
            target.meta.innerHTML = meta(`<span class="muted">Drawing unavailable (${r.status}).</span>`);
            return;
        }
