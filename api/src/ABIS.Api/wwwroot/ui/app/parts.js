...
async function renderRoutings(partId) {
    const body = $('#routingBody');
    if (partId == null) {
        body.innerHTML = '<p class="muted">Load a part to see and edit its routings.</p>';
        return;
    }
    try {
        const rows = (await authFetch(`/api/parts/${partId}/routings`).then((r) => r.json()));
      ...
      <div class="frow" style="margin-top:10px;align-items:flex-end">
        ...
        body.querySelectorAll('[data-del-routing]').forEach((b) => b.addEventListener('click', () => void removeRouting(partId, b.getAttribute('data-del-routing') || '')));
...
async function addRouting(partId) {
    ...
    if (need.some((id) => !v(id))) {
        msg.textContent = 'Seq, line, die, shape, SPM std/plan and people are required.';
        msg.className = 'err';
        return;
    }
    try {
...
async function removeRouting(partId, key) {
    const [seq, line, die, shape] = key.split('|');
    if (!window.confirm(`Remove routing #${seq} (line ${line} / die ${die} / ${shape})?`))
        return;
    try {
...
async function obsoletePart() {
    if (editingId == null) {
        setErr('Load a part first to obsolete it.');
        return;
    }
    setErr('');
    setOk('');
    let warning = '';
    try {
        const open = await (await authFetch(`/api/parts/${editingId}/order-items`)).json();
        if (open.length) {
            warning = [
                '',
    ...
    if (!window.confirm(`Obsolete part #${editingId}?${warning}`))
        return;
    setBusy(true);
    try {
...
async function revisePart() {
    if (editingId == null) {
        setErr('Load a part first to revise it.');
        return;
    }
    if (!window.confirm(`Do you want to create a revision of part #${editingId}?`))
        return;
    ...
    setBusy(true);
    try {
        const post = (routingSequence) => authFetch(`/api/parts/${editingId}/revise`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
        ...
        if (r.status === 409) {
            ...
            if (b?.code !== 'routing-choice-required' || !b.routings?.length) {
                setErr(b?.message ?? 'Revise failed (409).');
                return;
            }
            const list = b.routings.map((x) => `  ${x.routingSequence} - ${x.dieName ?? '?'} on ${x.lineDesc ?? '?'}`);
            const picked = window.prompt([`This part has ${b.routings.length} routings. Which one moves to the revision?`, '', ...list, '',
        ...
        await search();
        if (body.part?.partNumId)
            await loadPart(body.part.partNumId);
