...
public sealed class SheetSkid
{
    ...
    /// <summary>The skid's theoretical (calculated) weight, as opposed to the weighed
    /// <see cref="SheetNetWt"/> — legacy's modify screen edits both.</summary>
    public decimal? SheetTheoreticalWt { get; set; }
    /// <summary>Why the skid is on hold (<c>sheet_skid.onhold_reason_code</c>).</summary>
    public int? OnholdReasonCode { get; set; }
}
...
/// <summary>What an operator may change on a sheet skid, mirroring legacy's modify screen
/// (<c>w_office_skid_entry</c> CASE 4). Every field is optional: omit one and it is left alone, which
/// is what makes this a correction tool rather than a form that must be filled in again.</summary>
public sealed class SheetSkidModify
{
    public decimal? SheetNetWt { get; set; }
    public decimal? SheetTareWt { get; set; }
    public int? SkidPieces { get; set; }
...
/// <summary>Outcome of a sheet-skid modify: the skid as it now stands, plus any way its restated
/// totals disagree with the items on it. The disagreement is reported, never corrected.</summary>
public sealed class SheetSkidModifyResult
{
    public bool Found { get; set; }
    public SheetSkid? Skid { get; set; }
    public List<string> Warnings { get; set; } = new();
}
