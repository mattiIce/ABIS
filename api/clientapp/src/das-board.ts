...
function renderStrip(lines: Line[], liveKnown: boolean): void {
  ...
  const kpiLive = (n: number, label: string, cls = '') =>
    (liveKnown ? kpi(String(n), label, cls) : `<div class="das-kpi" title="The live line board could not be read, so this is unknown — not zero"><div class="n muted">—</div><div class="l">${label}</div></div>`);
  $('#strip').innerHTML =
...
//
// It reports WHY it has no rows, because "no lines are running" and "we could not find out" are
// different facts and this board is read from across the floor. Returning [] for both made a failed
// request render as "Running 0 · Stopped 0 · Open shifts 0" — a confident claim that the plant is
// idle, produced by a request that never arrived.
type LineBoardFetch = { live: LineBoard[]; known: boolean };

async function fetchLineBoard(): Promise<LineBoardFetch> {
  try {
    ...
    if (!r.ok) return { live: [], known: false };
    return { live: (await r.json()) as LineBoard[], known: true };
  } catch { return { live: [], known: false }; }
}
