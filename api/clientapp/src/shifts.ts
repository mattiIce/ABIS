 ...
 */
async function showStaleOpen(): Promise<void> {
  setErr(''); setBusy(true);
  try {
    const r = await authFetch('/api/das/shifts/open?staleOnly=true&boardOnly=true');
    if (!r.ok) throw new Error(`open-shift lookup failed (${r.status})`);
    const rows = await r.json() as Array<{
      shiftNum: number; lineNum?: number; lineDesc?: string; startTime?: string;
      operatorInitial?: string; hoursOpen?: number; coilRuns?: number;
    }>;
    ...
    if (dtHead) dtHead.textContent = 'Coil runs';
...
(async () => {
  ...
  if (new URLSearchParams(location.search).get('open') === 'stale') await showStaleOpen();
  else await search();
})();
