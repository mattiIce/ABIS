...
const setBusy = (b: boolean) => document.body.classList.toggle('scan-busy', b);
const setMsg = (m: string, kind: '' | 'err' | 'ok' = '') => { const el = $('#msg'); el.textContent = m; el.className = `scan-msg ${kind}`; };
const num = (n: number | undefined): string => (n == null ? '—' : n.toLocaleString());
const dt = (d: Date | undefined): string => (d == null ? '—' : d.toLocaleDateString());

let current: Coil | null = null;
...
function keepFocus(): void { const s = $<HTMLInputElement>('#scan'); s.focus(); s.select(); }

async function lookup(): Promise<void> {
  const raw = $<HTMLInputElement>('#scan').value.trim().replace(/[^0-9]/g, '');
  if (!raw) { setMsg('Scan or type a coil ID.', 'err'); return; }
  ...
  try {
    ...
    $<HTMLButtonElement>('#btnPrintTop').disabled = false;
    setMsg('');
...
function renderVerify(c: Coil): void {
...
async function printLabel(): Promise<void> {
  if (!current?.coilAbcNum) return;
  setBusy(true); setMsg('Fetching label…');
  try {
    const r = await authFetch(`/api/documents/coil-label/${current.coilAbcNum}`);
    if (!r.ok) { setMsg(`Label failed: ${r.status}`, 'err'); return; }
...
function reset(): void {
  ...
  $<HTMLButtonElement>('#btnPrintTop').disabled = true;
  $<HTMLInputElement>('#scan').value = '';
  setMsg('');
  keepFocus();
}
...
(async () => {
  ...
  scan.addEventListener('keydown', (e) => { if ((e as KeyboardEvent).key === 'Enter') { e.preventDefault(); void lookup(); } });
  ...
  document.addEventListener('click', (e) => { if (!(e.target as HTMLElement).closest('button, a, input')) keepFocus(); });
  keepFocus();
})();
