...
const num = (v: number | undefined | null): string => (v ?? 0).toLocaleString();
const $ = (root: HTMLElement, sel: string): HTMLElement => root.querySelector(sel) as HTMLElement;

function scaffold(): string {
  const kpi = (id: string, lbl: string) =>
    `<div class="kpi"><div class="k-lbl">${lbl}</div><div class="k-val skeleton" id="k-${id}">—</div><div class="k-foot flat" id="f-${id}"></div></div>`;
  return `
  <div class="page">
    <div class="page-head">
      <div>
...
function setKpi(main: HTMLElement, id: string, val: string, foot?: string, cls?: string): void {
  const v = $(main, `#k-${id}`); v.textContent = val; v.classList.remove('skeleton');
  if (foot !== undefined) { const f = $(main, `#f-${id}`); f.textContent = foot; if (cls) f.className = `k-foot ${cls}`; }
}

function faChip(status: number | undefined): string {
  return (status ?? 0) >= 1
    ? '<span class="chip ok">FA received</span>'
    : '<span class="chip warn">awaiting FA</span>';
}
function partnerPill(name: string | undefined): string {
  const t = (name ?? '?').replace(/[^A-Za-z0-9]/g, '').slice(0, 3).toUpperCase() || '?';
  let h = 0; for (const c of t) h = (h * 31 + c.charCodeAt(0)) % 360;
  return `<span class="pv" style="background:hsl(${h} 45% 38%)">${esc(t)}</span>`;
}

async function load(main: HTMLElement): Promise<void> {
  ...
  if (coils.status === 'fulfilled') setKpi(main, 'coils', num(coils.value.totalCount), 'in inventory', 'flat');
  else setKpi(main, 'coils', '—');
  ...
  } else setKpi(main, 'hold', '—');

  if (jobs.status === 'fulfilled') setKpi(main, 'jobs', num(jobs.value.totalCount), 'on file', 'flat');
  else setKpi(main, 'jobs', '—');
  ...
  } else { setKpi(main, 'sched', '0'); $(main, '#sched-body').innerHTML = '<p class="muted">Registry not provisioned.</p>'; }
  ...
  if (edi.status === 'fulfilled') {
    ...
    setKpi(main, 'edi', '—'); setKpi(main, 'fa', '—');
