...
import { AbisClient, SupervisorOverrideRequest } from './generated/abis-client.js';
 ...
 * dividing by it.
 */
export function endCoilBalancePercent(
  startingNetWt: number | null | undefined,
  skidTotal: number | null | undefined,
  scrapTotal: number | null | undefined,
  weightLeftOnCoil: number | null | undefined,
): number | null {
...
export function needsBalanceOverride(percent: number | null): boolean {
  return percent != null && percent > END_COIL_BALANCE_TOLERANCE_PERCENT;
}

export interface OverrideContext {
  action: 'end-coil-out-of-balance' | 'shift-override' | 'operation-panel' | 'offline-sheet';
  /** Shown at the top of the pad, so the supervisor is told what they are authorising. */
  prompt: string;
  lineNum?: number | null;
  abJobNum?: number | null;
  coilAbcNum?: number | null;
  /** Why it is needed — for the balance override, the discrepancy itself. Goes into the audit row. */
  reason?: string | null;
}
 ...
 * error to retry past.
 */
export function requestSupervisorOverride(ctx: OverrideContext): Promise<number | null> {
  return new Promise((resolve) => {
    ...
    const $ = <T extends HTMLElement>(sel: string) => back.querySelector(sel) as T;
    const loginEl = $<HTMLInputElement>('#supLogin');
    const pinEl = $<HTMLInputElement>('#supPin');
    const msg = $<HTMLElement>('#supMsg');
    let pin = '';

    const draw = () => { pinEl.value = '•'.repeat(pin.length); };
    const say = (m: string) => { msg.textContent = m; };
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (document.activeElement === loginEl && e.key !== 'Enter') return;
      if (e.key === 'Enter') { e.preventDefault(); void submit(); return; }
      if (/^[0-9]$/.test(e.key)) { e.preventDefault(); if (pin.length < 12) pin += e.key; draw(); }
      else if (e.key === 'Backspace') { e.preventDefault(); pin = pin.slice(0, -1); draw(); }
    };
    document.addEventListener('keydown', onKey, true);

    back.querySelectorAll<HTMLButtonElement>('.sup-pad button').forEach((b) => {
      b.addEventListener('click', () => {
        if (b.dataset.act === 'clear') pin = '';
        else if (b.dataset.act === 'back') pin = pin.slice(0, -1);
        else if (b.dataset.d && pin.length < 12) pin += b.dataset.d;
        draw();
        say('');
      });
    });

    $<HTMLButtonElement>('#supCancel').addEventListener('click', () => close(null));

    async function submit(): Promise<void> {
      const loginId = loginEl.value.trim();
      if (!loginId) { say('Enter the supervisor’s login.'); loginEl.focus(); return; }
      if (pin.length < 4) { say('Enter the PIN.'); return; }
      say('Checking…');
      try {
        const body = { action: ctx.action, loginId, pin, panel: panelLabel(),
          lineNum: ctx.lineNum ?? undefined, abJobNum: ctx.abJobNum ?? undefined,
          coilAbcNum: ctx.coilAbcNum ?? undefined, reason: ctx.reason ?? undefined } as SupervisorOverrideRequest;
        const r = await new AbisClient('', { fetch: authFetch }).requestSupervisorOverride(body);
        if (r.granted && r.overrideId != null) { close(r.overrideId); return; }
        ...
        say(r.lockedUntilUtc
          ? `Locked out until ${new Date(r.lockedUntilUtc as unknown as string).toLocaleTimeString()}.`
          : r.message || 'That PIN was not accepted.');
      } catch (e) {
        pin = ''; draw();
        say(`Could not check the PIN: ${(e as Error).message}`);
      }
    }

    $<HTMLButtonElement>('#supOk').addEventListener('click', () => void submit());
    loginEl.focus();
  });
}
