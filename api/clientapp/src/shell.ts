...
function wireUserMenu(): void {
  ...
  pop.querySelectorAll<HTMLButtonElement>('button[data-login]').forEach((b) => b.addEventListener('click', async () => {
    const l = b.dataset.login ?? '';
    // Sign in as the picked user (passwordless on the LAN); fall back to dev impersonation.
    try { await loginWithUser(l); localStorage.removeItem('abis_act_as'); }
    catch { localStorage.setItem('abis_act_as', l); }
    location.reload();
  }));
...
function loginGate(): Promise<void> {
  return new Promise((resolve) => {
    ...
    gate.addEventListener('keydown', (e) => { if ((e as KeyboardEvent).key === 'Enter') void enter(); });
 ...
 */
export function versionBadge(raw: string | undefined | null): string {
