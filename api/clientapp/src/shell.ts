...
export interface ShellOptions {
  active: string;
  /** When true, the page's existing body content is wrapped and moved into the shell
   *  (legacy-compat migration); the page keeps its own markup + logic. When false/absent,
   *  the caller renders fresh markup into the returned <main>. */
  adopt?: boolean;
}
...
function initTheme(): void {
  const saved = localStorage.getItem('abis_theme') as 'light' | 'dark' | null;
...
// ---- RBAC nav gating ----
async function gateNav(): Promise<{ name: string; role: string }> {
  ...
  try {
    ...
    if (perms && perms.length) {
      ...
      document.querySelectorAll<HTMLAnchorElement>('#shNav a[data-feature]').forEach((a) => {
  ...
  document.querySelectorAll<HTMLElement>('#shNav [data-group]').forEach((g) => {
    let n = g.nextElementSibling as HTMLElement | null, any = false;
    while (n && !n.hasAttribute('data-group')) { if (!n.hidden) any = true; n = n.nextElementSibling as HTMLElement | null; }
    g.hidden = !any;
  });
  return identity;
}
...
function wireUserMenu(): void {
  ...
  pop.querySelectorAll<HTMLButtonElement>('button[data-login]').forEach((b) => b.addEventListener('click', () => {
    const l = b.dataset.login ?? '';
    if (l) localStorage.setItem('abis_act_as', l); else localStorage.removeItem('abis_act_as');
    location.reload();
  }));
  pop.querySelector('#shSignout')!.addEventListener('click', () => {
    localStorage.removeItem('abis_act_as'); sessionStorage.removeItem('abis_entered'); location.reload();
  });
}

// ---- login gate ----
function loginGate(): Promise<void> {
  return new Promise((resolve) => {
    if (sessionStorage.getItem('abis_entered')) { resolve(); return; }
    ...
    const enter = () => {
      const key = (gate.querySelector('#lgKey') as HTMLInputElement).value.trim();
      const user = (gate.querySelector('#lgUser') as HTMLInputElement).value.trim();
      if (key) localStorage.setItem('abis_api_key', key);
      if (user) localStorage.setItem('abis_act_as', user); else localStorage.removeItem('abis_act_as');
      sessionStorage.setItem('abis_entered', '1');
      gate.remove(); resolve();
    };
    (gate.querySelector('#lgGo') as HTMLButtonElement).addEventListener('click', enter);
    gate.addEventListener('keydown', (e) => { if ((e as KeyboardEvent).key === 'Enter') enter(); });
    (gate.querySelector('#lgUser') as HTMLInputElement).focus();
  });
}
...
export async function initShell(opts: ShellOptions): Promise<HTMLElement> {
  ...
  const rail = document.createElement('aside'); rail.className = 'rail'; rail.innerHTML = railHtml(opts.active);
  const top = document.createElement('header'); top.className = 'top'; top.innerHTML = topHtml();
  ...
  applyTheme(document.documentElement.getAttribute('data-theme') as 'light' | 'dark');

  // interactions
  const toggle = () => rail.classList.toggle('collapsed');
  (top.querySelector('#shMenu') as HTMLButtonElement).addEventListener('click', toggle);
  (rail.querySelector('#shCollapse') as HTMLButtonElement).addEventListener('click', toggle);
  (top.querySelector('#shTheme') as HTMLButtonElement).addEventListener('click', () => {
  ...
  (top.querySelector('#shAvatar') as HTMLElement).textContent = initials(id.name);
