...
export interface ExportTable {
  /** Sheet name and the stem of the download filename — must identify WHICH job/coil this is. */
  name: string;
  headers: string[];
  rows: Cell[][];
}
...
interface ReportRowLike {
  coilAbcNum?: number; coilOrgNum?: string; lotNum?: string; alloy?: string;
  productType?: string; coilWt?: number; shipWt?: number; scrapWt?: number;
  rejectedWt?: number; yield?: number;
  coilRejected?: number; coilRebanded?: number; specialAttention?: number; specialHandling?: number;
}

interface DefectRowLike { scrapCode?: string; scrapDefect?: string; netWt?: number; pieces?: number; pct?: number }

interface WorksheetLike {
  abJobNum: number; coilAbcNum: number; customerId: number; autoparts: boolean; source: string;
  rows: { scrapCode?: string; scrapDefect?: string; netWt: number; pieces: number }[];
}

const n = (v: number | undefined): Cell => (v ?? null);
 ...
 */
export function flagText(r: ReportRowLike): Cell {
  const f: string[] = [];
  if (r.coilRejected) f.push('rejected');
  if (r.coilRebanded) f.push('rebanded');
  if (r.specialAttention) f.push('attention');
  if (r.specialHandling) f.push('handling');
  return f.length ? f.join(', ') : null;
}

/** The daily recovery report — the table legacy's Print button printed. */
export function reportTable(abJobNum: number, rows: ReportRowLike[]): ExportTable {
  return {
...
export function defectTable(abJobNum: number, rows: DefectRowLike[]): ExportTable {
  return {
 ...
 */
export function worksheetTable(w: WorksheetLike): ExportTable {
