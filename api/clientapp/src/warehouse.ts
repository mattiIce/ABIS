 ...
 */
async function createWarehouseSkid(): Promise<void> {
  const note = (m: string) => { $('#newOk').textContent = m; };
  setErr(''); note('');
  ...
  try {
    ...
    if (!r.ok) {
      ...
      let why = `${r.status}`;
      try { const p = await r.json() as { detail?: string; title?: string }; if (p?.detail || p?.title) why = p.detail ?? p.title!; }
      catch { /* not problem+json — keep the status */ }
      setErr(`Could not warehouse it in: ${why}`);
      return;
    }
    const created = await r.json() as { sheetSkidNum: number; coilAbcNum: number; coilMinted: boolean; warnings?: string[] };
...
async function deleteWarehouseSkid(): Promise<void> {
  if (editingNum == null) { setErr('Select a skid first.'); return; }
  if (!window.confirm(`Delete warehoused skid #${editingNum}? Its production items go with it, and its warehouse coil is removed if nothing else is on it.`)) return;
  setErr(''); setOk(''); setBusy(true);
  try {
    const r = await authFetch(`/api/warehouse/skids/${editingNum}`, { method: 'DELETE' });
    if (r.status === 404) { setErr(`Skid #${editingNum} no longer exists.`); return; }
    if (!r.ok) { setErr(`Delete failed (${r.status}).`); return; }
    const d = await r.json() as { coilRemoved?: boolean; coilAbcNum?: number; coilKeptReason?: string };
