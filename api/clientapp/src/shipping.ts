...
interface PackItem {
  shPackingItem: number; sheetSkidNum: number; skidDisplayNum?: string; netWeight: number;
...
async function loadPackingItems(id: number): Promise<void> {
  try {
    ...
    if (!r.ok) { $('#packItems').innerHTML = `<tr><td colspan="8" class="muted">Failed to load items (${r.status}).</td></tr>`; return; }
    ...
    document.querySelectorAll<HTMLButtonElement>('#packItems button[data-rm]').forEach((b) =>
      b.addEventListener('click', () => void removePackItem(Number(b.dataset.rm))));
...
async function removePackItem(shPackingItem: number): Promise<void> {
  if (selected == null) return;
  if (!window.confirm(`Remove item ${shPackingItem} from packing list ${selected}?`)) return;
  setErr('');
  try {
