...
type Problem = { title?: unknown; detail?: unknown; errors?: unknown; status?: unknown };
...
function parseProblem(body: unknown): Problem | undefined {
  if (typeof body !== 'string' || body.trim() === '') return undefined;
  try {
    const p: unknown = JSON.parse(body);
    return p !== null && typeof p === 'object' && !Array.isArray(p) ? (p as Problem) : undefined;
  } catch { return undefined; }   // an HTML error page from a proxy, say — not ours to show
}
...
export function problemText(e: unknown): string {
  if (typeof e === 'string') return nonEmpty(e) ?? 'Unknown error.';
  if (e === null || typeof e !== 'object') return e === undefined || e === null ? 'Unknown error.' : String(e);

  const x = e as { status?: unknown; response?: unknown; message?: unknown };
  // An ApiException carries the body as text; a declared ProblemDetails response IS the problem.
  const problem = parseProblem(x.response) ?? (e instanceof Error ? undefined : (e as Problem));
  ...
  if (status !== undefined && STATUS_TEXT[status]) return STATUS_TEXT[status];
  ...
  if (status !== undefined) return `The request failed (HTTP ${status}).`;
