...
interface ThrownShape extends ProblemShape {
  status?: number;
  response?: string;
  message?: string;
}
...
export function problemText(e: unknown): string {
  ...
  const x = (e ?? {}) as ThrownShape;

  let p: ProblemShape = x;
  if (typeof x.response === 'string' && x.response.trim()) {
    try { p = JSON.parse(x.response) as ProblemShape; } catch { /* not a problem body */ }
  }
  ...
  if (x.status === 403) return "You don't have permission to do that.";
  if (x.status === 404) return 'Not found.';
  if (p.title) return p.title;
  if (typeof x.status === 'number') return `The request failed (${x.status}).`;
