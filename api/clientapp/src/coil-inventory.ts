...
async function loadCoil(id: number): Promise<void> {
  ...
  try {
    ...
    const track: any = histRes.ok ? await histRes.json() : { entries: [] };
    const trackRows: any[] = track.entries ?? [];
    const firstAt = [
...
async function archivedBols(): Promise<void> {
  const cust = val('#fCustomer');
  $('#asnCoils').innerHTML = '';
  if (!cust) { $('#asnList').innerHTML = '<p class="muted">Enter a customer id in the search above first.</p>'; return; }
  setErr('');
  const qs = new URLSearchParams({ customerId: cust, pageSize: '50' });
  if (val('#aBol')) qs.set('bol', val('#aBol'));
  try {
    const r = await authFetch(`/api/inbound-asns?${qs.toString()}`);
    if (!r.ok) { setErr(`Archived BOLs failed (${r.status}).`); return; }
    const page = await r.json();
    const items: any[] = page.items ?? [];
    ...
    document.querySelectorAll<HTMLTableRowElement>('#asnList tr.click').forEach((tr) =>
      tr.addEventListener('click', () => void asnCoils(Number(tr.dataset.fid), tr.dataset.bol ?? '')));
  } catch (e) { setErr(`Archived BOLs failed: ${(e as Error).message}`); }
}

async function asnCoils(ediFileId: number, bol: string): Promise<void> {
  setErr('');
  try {
    const r = await authFetch(`/api/inbound-asns/${ediFileId}/coils?bol=${encodeURIComponent(bol)}`);
    if (!r.ok) { setErr(`ASN coils failed (${r.status}).`); return; }
    const coils: any[] = await r.json();
  ...
  } catch (e) { setErr(`ASN coils failed: ${(e as Error).message}`); }
}
