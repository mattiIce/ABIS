...
const refilters = new WeakMap<HTMLTableElement, () => void>();
...
const isDataRow = (tr: HTMLTableRowElement): boolean => !tr.querySelector('td[colspan], th[colspan]');
const cellText = (tr: HTMLTableRowElement, col: number): string => (tr.cells[col]?.textContent ?? '').trim();
...
const looksDate = (s: string): boolean => /\d{4}-\d\d-\d\d|\d{1,2}[/]\d{1,2}[/]\d{2,4}/.test(s);

function compare(a: string, b: string, numeric: boolean): number {
  if (numeric) {
    const na = toNum(a), nb = toNum(b);
    if (na != null && nb != null) return na - nb;
    if (na != null) return -1;      // real numbers before blanks/dashes
    if (nb != null) return 1;
...
function sortBy(table: HTMLTableElement, col: number, dir: 'ascending' | 'descending', numeric: boolean): void {
...
export function enhanceTable(table: HTMLTableElement): void {
  ...
  const tbody = table.tBodies[0];
  if (!thead || !thead.rows.length || !tbody) return;   // needs a header + body (skips key/value panels)
  table.dataset.enh = '1';
  ...
  table.addEventListener('click', (e) => {
    ...
    if (!th || th.parentElement !== headRow) return;
    const col = [...headRow.cells].indexOf(th as HTMLTableCellElement);
    if (col < 0) return;
 ...
 */
export function observeTables(root: HTMLElement): void {
  root.querySelectorAll<HTMLTableElement>('table.tbl').forEach(enhanceTable);
  const obs = new MutationObserver((records) => {
    const reload = new Set<HTMLTableElement>();
    for (const rec of records) {
      rec.addedNodes.forEach((n) => {
        if (!(n instanceof HTMLElement)) return;
        if (n.matches('table.tbl')) enhanceTable(n as HTMLTableElement);
        n.querySelectorAll<HTMLTableElement>('table.tbl').forEach(enhanceTable);
      });
      // Rows re-rendered into an enhanced table's <tbody> → re-apply its active filter.
      const tbl = (rec.target as HTMLElement)?.closest?.('table.tbl[data-enh]') as HTMLTableElement | null;
      if (tbl) reload.add(tbl);
    }
