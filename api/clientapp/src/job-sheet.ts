...
import { AbisClient, JobSheet, JobSheetDimension } from './generated/abis-client.js';
...
const dash = '—';
const txt = (v: unknown): string => (v == null || v === '' ? dash : esc(v));

/** A number at up to `dp` decimals, with trailing zeros trimmed the way the sheet reads. */
function num(v: number | undefined | null, dp = 3): string {
  if (v == null) return dash;
  const s = v.toFixed(dp).replace(/\.?0+$/, '');
  return s === '' || s === '-' ? '0' : s;
}

const dateOnly = (d: unknown): string => (d == null ? dash : new Date(d as string).toLocaleDateString());
 ...
 */
function dim(d: JobSheetDimension | undefined): string {
  if (!d) return dash;
...
const dimLabel = (d: JobSheetDimension | undefined, fallback: string): string =>
  !d?.name ? fallback : d.name.replace(/([A-Z])/g, (c) => ' ' + c.toLowerCase()).replace(/^./, (c) => c.toUpperCase());

function warnings(s: JobSheet): string {
  const rows: string[] = [];
  if (s.trimmingRequired) rows.push('<div class="js-warn">EDGE TRIMMING REQUIRED</div>');
...
function coilRows(s: JobSheet): string {
  const list = s.coils ?? [];
  if (!list.length) return `<tr><td colspan="${s.byLot ? 6 : 4}" class="muted">No coils on this job yet.</td></tr>`;
...
function partialRows(s: JobSheet): string {
...
function numbered(title: string, items: (string | undefined)[]): string {
...
export function jobSheetHtml(s: JobSheet): string {
  ...
  <h3>Piece weight</h3>
  <div class="js-pcwt">
    <span class="js-cell"><small>Gauge</small>${dim({ value: s.gauge, plusTol: s.gaugePlus, minusTol: s.gaugeMinus } as JobSheetDimension)}</span>
    ...
    <span class="js-cell"><small>Pitch</small>${dim({ value: s.pitch, plusTol: s.pitchPlus, minusTol: s.pitchMinus } as JobSheetDimension)}</span>
  </div>
...
function ensureStyles(): void {
  if (document.getElementById('job-sheet-css')) return;
 ...
 */
export async function renderJobSheet(container: HTMLElement, jobNum: number | null | undefined): Promise<JobSheet | null> {
  ensureStyles();
  if (jobNum == null) { container.innerHTML = '<p class="muted">No job selected.</p>'; return null; }
  ...
  try {
    ...
    const msg = (e as { status?: number }).status === 404
      ? `No job sheet for job ${jobNum}.`
      : `Job sheet unavailable: ${(e as Error).message}`;
 ...
 */
export function printJobSheet(sheet: JobSheet): void {
