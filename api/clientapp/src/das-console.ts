...
async function pullWeight(): Promise<void> {
  ...
  for (let i = 0; i < bases.length; i++) {
    try {
      ...
      if (!r.ok) continue;
      const reading = await r.json() as { value?: number; unit?: string; stable?: boolean; mode?: string };
      ...
      if (reading.mode === 'GS') {
        ...
        const tare = v('#skTare');
        if (!tare) {
          setErr('Scale is in GROSS mode — enter the skid tare first, or switch the indicator to NET.'); return;
        }
