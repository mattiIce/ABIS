...
async function loadJob(): Promise<void> {
  ...
  } catch (e) { setErr(`Load job failed: ${problemText(e)}`); }
...
async function queueWrite(path: string, method: string, body: unknown, okMsg: string): Promise<void> {
  ...
  try {
    ...
    await loadQueue();
  } catch (e) { setErr(`Queue: ${problemText(e)}`); }
  finally { setBusy(false); }
}
...
async function coilRunAction(path: string, body: unknown, okMsg: (r: RunResult) => string,
  ...
  try {
    ...
    then?.(res);
  } catch (e) { setErr(`Coil run: ${problemText(e)}`); }
  finally { setBusy(false); }
}
 ...
 */
async function coilBalancePercent(endWeight: number): Promise<number | null> {
  if (runCoil == null) return null;
  try {
    const r = await authFetch(`/api/das/coils/${runCoil}/balance`);
    if (!r.ok) return null;
    const b = await r.json() as { originalNetWt?: number | null; skidTotal?: number; scrapTotal?: number };
...
async function opAction(path: string, body: unknown, okMsg: string): Promise<void> {
  ...
  try {
    ...
    renderOpState();
    $('#opOk').textContent = okMsg;
  } catch (e) { setErr(`Operation panel: ${problemText(e)}`); }
  finally { setBusy(false); }
}
...
async function endShift(): Promise<void> {
  ...
  } catch (e) { setErr(`End shift: ${problemText(e)}`); }
  finally { setBusy(false); }
}
 ...
 */
async function zeroTheScale(): Promise<void> {
  if (!confirm('Re-zero the scale?\n\nMake sure nothing is on it — whatever is sitting there becomes the new zero.')) return;
  setErr(''); setOk('');
  const r = await zeroScale(parseEdgeUrls(v('#edgeUrl')));
  if (r.sent) setOk(`⚖ ${r.message}`); else setErr(r.message);
}
...
async function saveSkid(): Promise<void> {
  ...
  try {
    ...
    await loadSkids();
  } catch (e) { setErr(`Save skid failed: ${problemText(e)}`); }
  finally { setBusy(false); }
}
...
async function printDocument(url: string, label: string, noteSel: string): Promise<void> {
  ...
  } catch (e) { note(`Print failed: ${problemText(e)}`); }
}
...
async function pollCounters(bases: string[], runTag?: string): Promise<void> {
...
async function doScan(raw: string): Promise<void> {
  ...
  } catch (e) { setErr(`Scan failed: ${problemText(e)}`); scanned = null; }
  renderScan();
}
...
async function openAutoDowntime(): Promise<void> {
  ...
  } catch (e) { setErr(`Auto-downtime open failed: ${problemText(e)}`); }
}
...
async function logAutoDowntime(): Promise<void> {
  ...
  } catch (e) { setErr(`Log auto-downtime failed: ${problemText(e)}`); }
}
...
async function saveDowntime(): Promise<void> {
  ...
  } catch (e) { setErr(`Log downtime failed: ${problemText(e)}`); }
  finally { setBusy(false); }
}
