...
async function pullWeight(): Promise<void> {
  ...
  for (let i = 0; i < bases.length; i++) {
    try {
      ...
      if (!r.ok) continue;
      const reading = await r.json() as { value?: number; unit?: string; stable?: boolean; mode?: string; simulated?: boolean; device?: string };
      const from = i > 0 ? ' (fallback)' : '';
      if (reading.value == null) { setErr('The scale reported no weight — enter it manually.'); return; }
      ...
      if (reading.stable === false) {
        setErr('Scale is still settling — wait for it to steady, then pull again.'); return;
      }
      const unit = (reading.unit ?? '').toUpperCase();
      if (unit !== '' && unit !== 'LB' && unit !== 'LBS') {
        setErr(`Scale is reporting ${unit}, not pounds — correct the indicator's unit before weighing.`); return;
      }
      ...
      if (reading.mode === 'GS') {
        ...
        const tare = v('#skTare');
        if (!tare) {
          setErr('The indicator is reporting GROSS — enter the skid tare first, or switch it to NET.'); return;
        }
        const net = reading.value - Number(tare);
        if (!(net > 0)) {
          setErr(`Gross ${reading.value} is not more than the ${tare} tare — check the tare and the scale.`); return;
        }
        setV('#skNet', net);
