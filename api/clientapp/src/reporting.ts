...
interface Col { h: string; f: (r: any) => string; num?: boolean; raw?: (r: any) => unknown; }
...
const qwin = (f?: Date, t?: Date, extra: Record<string, string> = {}): string => {
  const q = new URLSearchParams(extra);
  if (f) q.set('from', f.toISOString());
  if (t) q.set('to', t.toISOString());
  const s = q.toString();
  return s ? `?${s}` : '';
};
// Shared column set for the three uptime groupings — a leading label column + the numeric hours.
const uptimeCols = (h: string, first: (r: any) => string): Col[] => [
  { h, f: first },
...
// Inputs a report needs beyond the date window. Only the two downtime reports below use them; the
// inputs are shown only while one of those reports is selected.
interface Params { causeId?: string; causeName?: string; jobA?: string; jobB?: string; }
type ParamKind = 'cause' | 'jobs';
const params = (): Params => ({
  causeId: $<HTMLSelectElement>('#pCause').value || undefined,
  causeName: $<HTMLSelectElement>('#pCause').selectedOptions[0]?.textContent ?? undefined,
  jobA: $<HTMLInputElement>('#pJobA').value.trim() || undefined,
  jobB: $<HTMLInputElement>('#pJobB').value.trim() || undefined,
});
...
async function compareJobs(p: Params): Promise<any[]> {
  if (!p.jobA || !p.jobB || !/^\d+$/.test(p.jobA) || !/^\d+$/.test(p.jobB))
    throw new Error('Enter two production order numbers to compare.');
  const [a, b] = await Promise.all([p.jobA, p.jobB].map((j) =>
    loadJson(`/api/reporting/downtime-pivot${qwin(undefined, undefined, { groupBy: 'cause', abJobNum: j })}`)));
  const rows = new Map<string, { bucket: string; a: number; b: number; aN: number; bN: number }>();
  const row = (k: string) => rows.get(k) ?? rows.set(k, { bucket: k, a: 0, b: 0, aN: 0, bN: 0 }).get(k)!;
  for (const r of a) { const x = row(r.bucket); x.a = r.downtimeMinutes; x.aN = r.occurrences; }
  for (const r of b) { const x = row(r.bucket); x.b = r.downtimeMinutes; x.bN = r.occurrences; }
  return [...rows.values()].sort((x, y) => Math.max(y.a, y.b) - Math.max(x.a, x.b));
}

const REPORTS: Record<string, { note: string; load: (from?: Date, to?: Date, p?: Params) => Promise<any[]>; cols: Col[]; params?: ParamKind }> = {
  summary: {
  ...
  'lbs-per-hour-month': {
    note:
      "Legacy's monthly MSR view: one row per line per month. Legacy's own SQL counts each shift's hours " +
  ...
  'dt-cause-by-day': {
    ...
    params: 'cause',
    load: (f, t, p) => {
      if (!p?.causeId) throw new Error('Pick a downtime cause.');
...
function syncParams(): void {
  const kind = REPORTS[$<HTMLSelectElement>('#report').value]?.params;
...
async function run(): Promise<void> {
  setErr(''); setBusy(true);
  currentKey = $<HTMLSelectElement>('#report').value;
...
const rawCell = (c: Col, r: any): unknown =>
  c.raw ? c.raw(r)
...
function exportCsv(): void {
  ...
  a.href = url; a.download = `${currentKey}-report.csv`; a.click();
...
function exportExcel(): void {
  ...
  const rows = current.map((r) => rep.cols.map((c) => rawCell(c, r) as (number | string | null)));
