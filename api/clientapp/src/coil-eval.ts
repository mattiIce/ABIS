...
async function loadDimChecks(): Promise<void> {
  const skid = v('#fSkid'); if (!skid) { $('#tDims').innerHTML = '<tr><td colspan="8" class="muted">Enter a skid #.</td></tr>'; return; }
...
async function loadEvalScrap(): Promise<void> {
...
async function addDimCheck(): Promise<void> {
  const skid = v('#fSkid'); if (!skid) { setErr('Enter a skid # first.'); return; }
  ...
  try {
    await client().createDimensionCheck(Number(skid), new DimensionCheckWrite({
      ...
      inSpec: $<HTMLInputElement>('#dInSpec').checked ? 1 : 0,
...
async function addScrap(): Promise<void> {
  if (job == null) return;
  setErr(''); setOk(''); setBusy(true);
  try {
...
async function init(): Promise<void> {
  ...
  $<HTMLFormElement>('#skidForm').addEventListener('submit', (e) => { e.preventDefault(); void loadDimChecks(); });
