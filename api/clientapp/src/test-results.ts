...
interface TestRow {
  coilAbcNum?: number; position?: string; testType?: number; createdDate?: string;
  ytsVal?: number; utsVal?: number; elongVal?: number; nVal?: number; rVal?: number; thickness?: number; width?: number;
}

function scaffold(): string {
  const cell = (id: string, label: string, w = 90, ph = '') =>
    `<div class="fld"><label>${label}</label><input id="${id}" inputmode="decimal" style="width:${w}px" placeholder="${ph}" /></div>`;
  return `
  <div class="page">
...
async function loadResults(): Promise<void> {
  ...
  if (val('#qPos')) qs.set('position', val('#qPos'));
  if (val('#qType')) qs.set('testType', val('#qType'));
  try {
    const r = await authFetch(`/api/test-results?${qs.toString()}`);
    if (!r.ok) { setErr(`Load failed (${r.status}).`); return; }
    const page = await r.json();
    const items: TestRow[] = page.items ?? [];
    const n = (v: unknown) => `<td class="mono" style="text-align:right">${esc(v ?? '')}</td>`;
...
async function addResult(): Promise<void> {
  ...
  const pos = val('#fPos');
  if (!coil || !pos) { setErr('Coil ABC # and Position are required.'); return; }
...
(async () => {
  ...
  $<HTMLFormElement>('#filterForm').addEventListener('submit', (e) => { e.preventDefault(); void loadResults(); });
  await loadResults();
})();
