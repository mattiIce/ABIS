...
async function loadPinCoverage(): Promise<void> {
  const box = $('#pinCov');
  const sub = $('#pinCovSub');
  try {
    const r = await authFetch('/api/security/supervisor-pin-coverage?group=IT');
    if (!r.ok) { sub.textContent = ''; box.innerHTML = '<span class="muted">Coverage unavailable.</span>'; return; }
    const c = await r.json() as {
      groupExists: boolean; activeWithPin: number; activeWithoutPin: number;
      members: { userId: number; loginId?: string; userName?: string; userStatus?: number; hasPin: boolean; lockedUntilUtc?: string | null }[];
    };
    if (!c.groupExists) {
    ...
    box.querySelectorAll<HTMLTableRowElement>('tr.click').forEach((tr) =>
      tr.addEventListener('click', () => void openUser(Number(tr.dataset.id))));
  } catch { sub.textContent = ''; box.innerHTML = '<span class="muted">Coverage unavailable.</span>'; }
}
