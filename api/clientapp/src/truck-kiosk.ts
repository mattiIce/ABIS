...
interface Appt {
  appointmentId: number; direction?: string; carrierName?: string; refType?: string; refId?: string;
  scheduledStart?: string; driverName?: string; driverPhone?: string;
  truckStatus: number; checkinTime?: string | null; checkoutTime?: string | null;
}

const api = (path: string, method = 'GET', body?: unknown): Promise<Response> =>
  authFetch(path, {
    method,
...
const setErr = (m: string) => { $('#k-err').textContent = m; };
const fmtWhen = (s?: string): string => (s ? new Date(s).toLocaleString(undefined, { weekday: 'short', hour: '2-digit', minute: '2-digit' }) : '—');
...
const bigBtn = (bg: string) => `width:100%;box-sizing:border-box;padding:18px;font-size:22px;font-weight:700;border:0;border-radius:14px;color:#fff;background:${bg};margin-top:14px;cursor:pointer`;
...
function renderLookup(): void {
  ...
  const find = () => void lookup(($('#k-ref') as HTMLInputElement).value.trim());
  $('#k-find').addEventListener('click', find);
  $('#k-ref').addEventListener('keydown', (e) => { if ((e as KeyboardEvent).key === 'Enter') find(); });
}

async function lookup(ref: string): Promise<void> {
  setErr('');
  if (!ref) { setErr('Type your BOL number or appointment number first.'); return; }
  try {
    const r = await api(`/api/truck-appointments/lookup?q=${encodeURIComponent(ref)}`);
    if (!r.ok) { setErr('Could not search right now — please see the gate.'); return; }
    const list = (await r.json()) as Appt[];
    if (!list.length) { setErr(`No appointment found for "${ref}". Check the number or see the gate.`); return; }
    if (list.length === 1) { renderAppt(list[0]); return; }
    renderPick(list);
  } catch { setErr('Could not reach the system — please see the gate.'); }
}

function renderPick(list: Appt[]): void {
  ...
  $('#k-body').querySelectorAll<HTMLButtonElement>('button[data-i]').forEach((b) =>
    b.addEventListener('click', () => renderAppt(list[Number(b.dataset.i)])));
  $('#k-back').addEventListener('click', renderLookup);
}

function renderAppt(a: Appt): void {
  setErr('');
  ...
  if (a.truckStatus === 9) { $('#k-body').innerHTML = details + msg('This appointment was cancelled — please see the gate.'); wireRestart(); return; }
  if (a.truckStatus === 6 || a.checkoutTime) { $('#k-body').innerHTML = details + msg('You’re signed out — safe travels! ✅'); wireRestart(); return; }
...
async function act(a: Appt, action: 'check-in' | 'check-out'): Promise<void> {
  setErr('');
  let body: unknown;
  if (action === 'check-in') {
    const phone = ($('#k-phone') as HTMLInputElement).value.trim();
    const name = ($('#k-name') as HTMLInputElement).value.trim();
    if (!phone) { setErr('Please enter your cell phone number so we can notify you.'); return; }
    body = { driverName: name || null, driverPhone: phone };
  }
  try {
    const r = await api(`/api/truck-appointments/${a.appointmentId}/${action}`, 'POST', body);
    if (!r.ok) { setErr(action === 'check-out' ? 'Could not check out — please see the gate.' : 'Could not check in — please see the gate.'); return; }
...
const msg = (html: string) => `<div style="${card};text-align:center;font-size:22px;font-weight:700;padding:28px 18px">${html}</div>`;
