$PBExportHeader$u_default_combi_1999_theo.sru
$PBExportComments$default abco packinglist & billoflading combination  form theorotical wt only
forward
global type u_default_combi_1999_theo from u_report_object
end type
end forward
...
global u_default_combi_1999_theo u_default_combi_1999_theo
...
Long ll_skid_num, ll_job_num, ll_theo_wt, ll_skidpc, ll_tarewt, ref_order
	...
	IF li_totaljob > 0 THEN
		FOR li_i = 1 TO li_totaljob
			...
			FOR li_s = 1 TO ids_sheet.RowCount()
				...
				SELECT ab_job_num, sheet_theoretical_wt, skid_pieces, sheet_tare_wt , ref_order_abc_num, ref_order_abc_item 
				INTO :ll_job_num, :ll_theo_wt, :ll_skidpc, :ll_tarewt, :ref_order, :ref_item
				FROM sheet_skid
				...
				IF istr_job_info[li_i].abco_num = ll_job_num and istr_job_info[li_i].ref_order_num = ref_order and istr_job_info[li_i].ref_order_item_num = ref_item THEN
					IF IsNULL(ll_theo_wt) THEN ll_theo_wt = 0
					...
					IF ll_itemrow > 0 THEN
						...
						lds_print.SetItem(ll_newrow, "grosswt_t", String((ll_theo_wt + ll_tarewt), "#,###,###"))
			...
			IF (li_totaljob > 1) OR (ids_rejcoil.RowCount() > 0)  OR (ids_scrap.rowCount() > 0) THEN
				...
				lds_print.SetItem(ll_newrow, "tarewt_t", String(ll_jobtare, "###,###,###") )
				lds_print.SetItem(ll_newrow, "tarewt_kg_t", String(ll_jobtare_kg, "###,###,###") )
				lds_print.SetItem(ll_newrow, "grosswt_t", String((ll_jobtheo+ll_jobtare), "###,###,###") )
				lds_print.SetItem(ll_newrow, "grosswt_kg_t", String((ll_jobtheo_kg +ll_jobtare_kg), "###,###,###") )
			END IF
			...
			IF li_i = li_totaljob THEN 
				...
				lds_print.SetItem(ll_newrow, "tarewt_t", String(il_shiptare, "###,###,###") )
				lds_print.SetItem(ll_newrow, "tarewt_kg_t", String(il_shiptare_kg, "###,###,###") )
				lds_print.SetItem(ll_newrow, "grosswt_t", String((il_shiptheo + il_shiptare), "###,###,###") )
				lds_print.SetItem(ll_newrow, "grosswt_kg_t", String((il_shiptheo_kg + il_shiptare_kg), "###,###,###") )
			END IF				
			//end of data
...
//MessageBox("of_fill_header for u_default_combi_1999_theo", "idw_requestor.DataObject = " + idw_requestor.DataObject)
...
//MessageBox("of_fill_print_header for u_default_combi_1999_theo", "ads_print.DataObject = " + ads_print.DataObject)
...
on u_default_combi_1999_theo.create
call super::create
end on

on u_default_combi_1999_theo.destroy
call super::destroy
end on
...
IF li_totalrow > 0 THEN //retrieve order information
	...
	FOR li_int = 1 TO li_totalrow
		...
		ll_job_num = 0
		SELECT ab_job_num, sheet_theoretical_wt, ref_order_abc_num, ref_order_abc_item INTO :ll_job_num, :ll_theo_wt, :ll_ref_order, :ll_item_num
		FROM sheet_skid
		WHERE sheet_skid_num = :ll_skid_num
		USING SQLCA;
		...
		IF li_index > 0 THEN
			FOR li_i = 1 TO li_index 
				IF istr_job_info[li_i].abco_num = ll_job_num and istr_job_info[li_i].ref_order_num = ll_ref_order and istr_job_info[li_i].ref_order_item_num = ll_item_num THEN
					istr_job_info[li_i].sub_total_net_wt = istr_job_info[li_i].sub_total_net_wt + ll_theo_wt
		...
		IF NOT(lb_exist) THEN
			...
			istr_job_info[li_index].sub_total_net_wt = ll_theo_wt
...
IF li_totalrow > 0 THEN 
	FOR li_int = 1 TO li_totalrow
		ll_nwt = 0
		ll_twt = 0
		ll_nwt = ids_sheet.GetItemNumber(li_int, "sheet_skid_sheet_theoretical_wt", Primary!, FALSE)
