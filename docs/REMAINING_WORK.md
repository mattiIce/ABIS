...
- [~] **C** Dimension-check tolerance validation vs part-shape spec (nominal ± tol) — the real QC gate. **UNBLOCKED via WinSPC:** the plant's WinSPC (SPC) DB is the QC system of record and already holds measured value + LSL/target/USL + pass/fail per characteristic, tied to ABIS by the "Job #"/"Coil #" tag. Phase 1 shipped (#229 read-only connector + #230 Dimensional QC page) — displays WinSPC QC by job/coil. **Phase 2 (pending live wiring):** drive `in_spec` from WinSPC once the read-only SQL login on RSEDAM-PC + network path from the ABIS host are set up and the mapping is validated against live data. (The original legacy `d_skid_dim_check` rule stays un-reconstructable, but is now moot.)
