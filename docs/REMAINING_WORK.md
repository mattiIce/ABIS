...
- [~] **H** Coil-quality capture + flaw mapping DONE (GET/PUT /coils/{n}/quality header + POST/DELETE .../quality/flaws — COIL_QUALITY + COIL_QUALITY_FLAW_MAPPING, fixture-only tables) + a **Coil quality** capture page (header form + flaw add/remove). Still TODO: QR/barcode capture into it + inbound status-on-receipt (received_time, damage codes).
